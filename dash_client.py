...
class DashClient:
    ...
    def __init__(self):
        ...
        r2a_class = getattr(importlib.import_module(
            'r2a.' + r2a_algorithm.lower()), r2a_algorithm)
    ...
    def handle_scheduler_event(self, event):
        ...
        if event.get_dst() < 0 or event.get_dst() >= len(self.modules):
            print(
                f'It is no possible to route a {event.get_msg()} message from {event.get_src()} to {event.get_dst()} module list position.')
            exit(0)
